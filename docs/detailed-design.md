...
トップレベルに `patterns` オブジェクトが必須。キーはグロブパターン（`.editorconfig` 方式）、値はそのパターンに一致するファイルへの設定。複数パターンが一致した場合は後勝ちでマージされる（**ファイル内で後に記述されたエントリが勝つ**）。

後勝ちの順序はファイルの記述順（ソース順）に厳密に従う。JavaScript のオブジェクトキー列挙順は整数風のキー（例: `"12"`）を記述位置に関係なく先頭へ昇順で並べるため、実装は `.rawimagerc` の生テキストを走査してソース順を決定する（走査に失敗した場合のみオブジェクト列挙順にフォールバック）。したがって `"*"` の後に `"12"` を書けば、両方に一致するファイルでは `"12"` の設定が勝つ。
