...
- Webview 側は起動直後に `ready` を送信し、以後 `render`/`error` のいずれかを受信するまで 250 ms 間隔で `ready` を再送し続ける（Extension 側でのメッセージ取りこぼしに備えるため）
- Extension 側は `ready` を受信した時点で初回の `render`（または設定解決に失敗していれば `error`）を送信する。フォールバック送信は行わない — `ready` を受信しない限り Extension 側から自発的に `render` を送ることはない
- `render`/`error` を受信した Webview 側は `ready` の再送を停止する
- タイムアウトは 2 段構えになっている（いずれも `ready`/`render` の往復が成立しなかった場合の検知用）:
  - Webview 側: `ready` 送信開始から 4 秒以内に `render`/`error` を受信しなければ、Webview 内にタイムアウトエラーを表示する
  - Extension 側: `render` を送信済みでない状態で 5 秒経過した場合、`ready` が届いていないと判断して警告メッセージ（VS Code の通知）を表示する
...
sequenceDiagram
  ...
  W->>E: ready（以後 250ms 間隔で render/error 受信まで再送）
  Note over E: 5秒経っても ready 未受信なら警告を表示
  Note over W: 4秒経っても render/error 未受信ならタイムアウトエラーを表示
