...
- CI（lint + compile + test）がすべてパスしていることを確認してください。
...
このファイルは `build-vsix.yml` の代替ではありません。`build-vsix.yml` は PR や push 時の検証用、
...
`.github/workflows/build-vsix.yml` により、`main` ブランチへの push とすべての PR に対して自動的に実行されます：
...
| Test                        | `xvfb-run -a npm test`     |
| Release metadata validation | `npm run validate:release` |
...
2. リリース対象を GitHub milestone または project にまとめる運用を推奨します。これにより、そのバージョンに含める Issue / PR を一覧で確認できます。

3. リリース前に、以下のバージョン情報を同期してください：
   - `package.json`
   - `package-lock.json`
   - `CHANGELOG.md`
   - Git タグ（`v<major>.<minor>.<patch>`）
...
5. GitHub Actions の `release.yml` ワークフローが自動実行され、以下が実行されます：
   - Lint・Compile チェック
   - リリースメタデータ検証（tag / package version / changelog）
   - VSIX ファイル生成
   - GitHub Release 作成
   - VSIX ファイルをリリースアセットとしてアップロード

6. [GitHub Releases](../../releases) ページで、リリースが表示されることを確認してください。

### バージョニング規則

- タグ形式は `v<major>.<minor>.<patch>` です（例：`v1.0.0`）。
- **Git タグ、`package.json` / `package-lock.json` の version、`CHANGELOG.md` の見出しは同じバージョンで一致している必要があります。**
- Release workflow は、タグと `package.json` の version が一致しない場合や、対応する `CHANGELOG.md` 見出しが存在しない場合に fail します。
