...
All notable changes to the "Raw Image Viewer: Pattern-based configuration" extension will be documented in this file.
...
- **Simplified the startup handshake**: removed the Extension-side 300ms fallback timer that force-sent the initial `render` even without a `ready` acknowledgement. The Webview's 250ms `ready` retransmission (until `render`/`error` is received) is now the sole mechanism for recovering from a lost first message; the Extension only sends `render` once `ready` is actually received. The Webview's 4s timeout error and the Extension's 5s `ready`-not-received warning are both unchanged
...
- The Webview's `render` handler cleaned up the previous ResizeObserver/pan listeners/fetch abort controller _after_ its early-return branches (`config: null`, missing `fileUri`, insufficient data), so a successful render followed by one of those branches leaked the old canvas's ResizeObserver and window listeners. The cleanup is now extracted into `releaseActiveRenderResources()` and runs before any early return, for both `render` and `error` messages
- `FileSystemWatcher`s for `.rawimagerc` were registered all the way up to the filesystem root for every opened file/tab, which could create 10-20+ watchers per tab. Watcher registration (`getConfigWatchDirectories()`) is now capped at the file's workspace folder root (inclusive); `.rawimagerc` _discovery_ (`findConfigPath()`) is unaffected and still walks to the filesystem root
- `.rawimagerc` `patterns` entries are now validated field-by-field before being merged, instead of only validating the final merged value. Previously, an invalid `width`/`height`/`headerSize`/`format` in an entry that got overridden by a later matching entry was silently ignored, and an explicit `"format": null` silently fell back to `rgb24` (asymmetric with `width`/`height`, which already rejected `null`). Error messages now include the offending entry's index, e.g. `patterns[1].format must be one of ...`
- Minor: the Webview now rejects odd-width/height YUV formats (`yuv420p`, `nv12`, `yuyv422`) before fetching the file instead of only failing during decode; `getNonce()` now uses `crypto.randomBytes()` instead of `Math.random()`; the Explorer context menu's **Open as Raw Image** entry no longer appears when right-clicking a folder; `.rawimagerc`'s schema now requires at least one `patterns` entry, and a couple of dead/redundant conditionals in `config.ts`'s fallback-config resolution were cleaned up
